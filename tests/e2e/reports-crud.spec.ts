/**
 * Tests E2E exhaustifs — Page /reports
...
/**
 * Sélectionne le premier vrai dashboard dans le select #f-dash s'il existe.
 * Le select est chargé dynamiquement depuis l'API — on attend que les options arrivent.
 */
async function selectFirstDashboardIfAvailable(page: Page) {
  ...
  try {
    ...
    await select.selectOption({ index: 1 })  // première option non-vide
  } catch {
    // Pas de dashboard disponible — laisser vide (champ optionnel)
  }
}

// ─── Suite 1 : Chargement ────────────────────────────────────────────────────

test.describe('/reports – Chargement et affichage', () => {
  test.beforeEach(async ({ page }) => {
    await login(page)
    await goToReports(page)
  })

  test('GET /api/visualizations/reports/ → 200 avec liste valide', async ({ page }) => {
  ...
  test('La vue liste affiche les colonnes attendues', async ({ page }) => {
    ...
    if (await table.isVisible()) {
      ...
      // Mode grille par défaut — vérifier les cards
...
test.describe('/reports – CREATE rapport', () => {
  test.beforeEach(async ({ page }) => {
    await login(page)
    await goToReports(page)
  })
  ...
  test('Le select #f-dash est un vrai <select> avec l\'option vide par défaut', async ({ page }) => {
    await page.locator('button', { hasText: /Nouveau rapport/i }).click()
    const drawer = page.locator('[role="dialog"]')
    await expect(drawer).toBeVisible()
  ...
  test('CREATE : le rapport apparaît dans la liste après création', async ({ page }) => {
  ...
  test('CREATE : les présets CRON remplissent correctement le champ schedule', async ({ page }) => {
    await page.locator('button', { hasText: /Nouveau rapport/i }).click()
    ...
    // Cliquer le préset "Quotidien" (class cron-preset-btn)
...
// ─── Suite 3 : GENERATE (téléchargement réel) ────────────────────────────────

/** Crée un rapport dans le format donné et retourne son ID */
...
test.describe('/reports – GENERATE (téléchargement fichier)', () => {
  test.beforeEach(async ({ page }) => {
    await login(page)
    await goToReports(page)
  })

  test('GENERATE CSV : POST → 200 + Content-Disposition attachment + extension .csv', async ({ page }) => {
    await createReport(page, 'csv')
    const generateBtn = page.locator('button[title*="Générer"]').first()
  ...
  test('GENERATE PDF : WeasyPrint → 200 + Content-Type application/pdf', async ({ page }) => {
  ...
  test('GENERATE : le clic déclenche un téléchargement de fichier physique (download event)', async ({ page }) => {
  ...
  test('GENERATE : le toast "Fichier téléchargé" apparaît après le download', async ({ page }) => {
    ...
    await expect(toast).toContainText(/Fichier téléchargé/i)
  })
})

// ─── Suite 4 : EDIT ──────────────────────────────────────────────────────────

test.describe('/reports – EDIT', () => {
  test.beforeEach(async ({ page }) => {
    await login(page)
    await goToReports(page)
  })

  test('EDIT : le bouton crayon ouvre le drawer avec le nom pré-rempli', async ({ page }) => {
  ...
  test('EDIT : PATCH /api/visualizations/reports/{id}/ → 200 avec données mises à jour', async ({ page }) => {
    const editBtn = page.locator('button[title="Modifier"]').first()
    await expect(editBtn).toBeVisible({ timeout: 8_000 })
    await editBtn.click()
    ...
    expect(res.status()).toBe(200)
    const body = await res.json()
    expect(body.name ?? body.data?.name).toContain('Modifié')
  })
})
...
test.describe('/reports – DELETE', () => {
  ...
  test('DELETE : DELETE /api/visualizations/reports/{id}/ → 204 et disparition', async ({ page }) => {
    const delName = `DEL E2E ${TS}`

    // Créer via le formulaire UI pour un test 100% E2E
    await page.locator('button', { hasText: /Nouveau rapport/i }).click()
    ...
    // Modale de confirmation → bouton .btn-danger
