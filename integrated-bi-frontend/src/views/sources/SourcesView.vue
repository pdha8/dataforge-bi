...
function openDrawer() {
  form.value = { name: '', source_type: 'postgresql', host: '', port: '5432', database_name: '', username: '', password: '', description: '', url: '' }
...
async function submitForm() {
  ...
  try {
    if (isFileType.value && selectedFile.value) {
      ...
      await api.post('/api/data-sources/sources/', {
...
<template>
  <div class="sources-page">
    ...
    <div class="stats-strip">
      ...
      <div class="stat-cell">
        <span class="stat-n stat-n--ok">{{ stats.active }}</span>
        <span class="stat-l">Actives</span>
      </div>
      <div class="stat-div"></div>
      <div class="stat-cell">
        <span class="stat-n stat-n--err">{{ stats.error }}</span>
    ...
    <Transition name="drawer-anim">
      <div v-if="drawerOpen" class="drawer-overlay" @click.self="closeDrawer">
        <aside class="drawer" role="dialog" aria-modal="true" aria-label="Nouvelle source de données">
          ...
          <!-- Drawer header -->
          <div class="drawer-hd">
            <h3 class="drawer-title">Nouvelle source</h3>
          ...
          <form class="drawer-form" @submit.prevent="submitForm">
            ...
            <template v-if="isApiType">
              ...
              <div class="form-field">
                <label class="form-label" for="f-apikey">
                  Clé API / Token <span class="opt">optionnel</span>
                </label>
                <input id="f-apikey" v-model="form.password" class="form-input" type="password" placeholder="Bearer token ou clé API" autocomplete="new-password" />
              </div>
            </template>
            ...
            <div class="drawer-footer">
              ...
              <button type="submit" class="btn-primary" :class="{ 'btn-primary--loading': submitting }" :disabled="submitting">
                <span v-if="!submitting">Créer la source</span>
