#!/usr/bin/env python
"""
SOTIFibre BI Platform — Script de peuplement des données de test
Usage: cd /home/adoum/sotifibre_backend_django
...
# LocMemCache (developpement) n'a pas delete_pattern (methode redis-only).
# On ajoute un no-op pour eviter les erreurs des signaux post_save.
if not hasattr(cache, 'delete_pattern'):
    cache.delete_pattern = lambda *a, **kw: None

print("\n" + "=" * 60)
print("  SOTIFibre BI Platform — Seeding des données de test")
print("=" * 60)
...
dev_user      = make_user('dev.bi@sotifibre.dz',      'dev_bi',       'Karim',   'Meziane',   'bi_developer', 'Département BI',   'Développeur BI')
analyst_user  = make_user('analyste@sotifibre.dz',    'analyste_bi',  'Nadia',   'Brahim',    'bi_analyst',   'Département BI',   'Analyste BI')
director_user = make_user('direction@sotifibre.dz',   'direction',    'Mohamed', 'Amrani',    'bi_consumer',  'Direction Générale','Directeur Général')
tech_user     = make_user('technicien@sotifibre.dz',  'technicien',   'Rachid',  'Hamidi',    'viewer',       'Technique',        'Technicien Réseau')
...
team_bi, c = Team.objects.get_or_create(name='Équipe BI Core', defaults=dict(
    description='Équipe centrale BI — développement et maintenance des tableaux de bord et KPIs SOTIFibre.',
    team_lead=admin_user,
))
if c:
    team_bi.members.set([admin_user, dev_user, analyst_user])
    print("  ✓ Équipe BI Core")

team_data, c = Team.objects.get_or_create(name='Équipe Data Engineering', defaults=dict(
    description='Équipe Data Engineering — pipelines ETL, Data Warehouse et infrastructure données.',
    team_lead=dev_user,
))
if c:
    team_data.members.set([dev_user, analyst_user, tech_user])
    print("  ✓ Équipe Data Engineering")

# ─────────────────────────────────────────────────────────────
# 3. SOURCES DE DONNÉES
# ─────────────────────────────────────────────────────────────
print("\n[3/14] Sources de données...")
...
src_pg = make_source(
    'Base de Données Production SOTIFibre',
    description='BD PostgreSQL principale — clients, contrats, équipements réseau et historique incidents.',
    ...
    category='Base de données', business_domain='Réseau Fibre',
    tags=['production', 'postgresql', 'fibre', 'principal'],
    owner=admin_user, created_by=dev_user,
)

src_iot = make_source(
    'API IoT Capteurs Réseau Fibre',
    description='API REST exposant les métriques temps réel des capteurs IoT sur les nœuds de distribution fibre : débit, latence, taux d\'erreur.',
    ...
    category='API', business_domain='Réseau Fibre',
    tags=['iot', 'capteurs', 'temps-réel', 'fibre'],
    owner=dev_user, created_by=dev_user,
)

src_csv = make_source(
    'Export Clients SOTIFibre CSV',
    description='Fichiers CSV exportés mensuellement depuis le CRM — abonnés fibre, offres et localisation.',
...
src_kafka = make_source(
    'Stream Kafka Alertes Sécurité Réseau',
    description='Topic Kafka collectant en temps réel les alertes sécurité réseau et anomalies détectées sur l\'infrastructure fibre.',
    ...
    category='Streaming', business_domain='Sécurité',
    tags=['kafka', 'alertes', 'sécurité', 'streaming'],
    owner=dev_user, created_by=dev_user,
)

src_meteo = make_source(
    'API Météo Nationale (Corrélation Pannes)',
    description='API météo pour corréler les incidents réseau fibre avec les conditions climatiques dans les zones de déploiement.',
    source_type='rest_api', status='active', api_type='rest',
    ...
    tags=['météo', 'corrélation', 'incidents', 'api'],
    owner=analyst_user, created_by=analyst_user,
)

# ─────────────────────────────────────────────────────────────
# 4. TABLES DE DONNÉES (sous la source PostgreSQL)
# ─────────────────────────────────────────────────────────────
print("\n[4/14] Tables de données...")

tables_spec = [
    ('clients', 'Abonnés SOTIFibre avec offres fibre actives', 42380, [
    ...
    ('incidents_reseau', 'Journal des incidents réseau fibre optique', 28640, [
        {'name': 'id', 'type': 'integer', 'nullable': False},
    ...
    ('consommation_bande_passante', 'Métriques de consommation bande passante par nœud', 1240000, [
    ...
    ('equipements_reseau', 'Catalogue des équipements réseau fibre déployés', 3840, [
        {'name': 'id', 'type': 'integer', 'nullable': False},
    ...
    ('contrats', 'Contrats abonnement fibre actifs et archivés', 45920, [
        {'name': 'id', 'type': 'integer', 'nullable': False},
...
for tname, tdesc, trows, tcols in tables_spec:
    ...
    if c: print(f"  ✓ {tname} ({trows:,} lignes)")

# ─────────────────────────────────────────────────────────────
# 5. DATA WAREHOUSE — SCHÉMA
# ─────────────────────────────────────────────────────────────
print("\n[5/14] Schéma Data Warehouse...")

dw_schema, c = DataWarehouseSchema.objects.get_or_create(
    name='sotifibre_dw',
    defaults=dict(
        description='Entrepôt de données centralisé SOTIFibre — schéma principal alimenté par les pipelines ETL.',
...
if c: print("  ✓ Schéma 'sotifibre_dw' créé")

# ─────────────────────────────────────────────────────────────
# 6. DATA WAREHOUSE — TABLES (Facts + Dimensions)
# ─────────────────────────────────────────────────────────────
...
fact_specs = [
    ('fact_incidents_reseau',   'transaction', 'Incidents réseau fibre — durée, sévérité, clients impactés', 28640),
    ('fact_consommation_bp',    'daily',       'Consommation bande passante agrégée par nœud/jour',         365000),
    ('fact_interventions',      'transaction', 'Interventions terrain — coûts, durées, résultats',          18720),
]
for tname, grain, tdesc, trows in fact_specs:
    ...
    dw_tables[tname] = t
    if c: print(f"  ✓ {tname} [FAIT]")

dim_specs = [
    ('dim_temps',        'slowly_changing', 'Dimension temps — hiérarchie date/heure',              3653),
    ('dim_client',       'slowly_changing', 'Dimension clients SOTIFibre — attributs abonnés',      42380),
    ('dim_equipement',   'conformed',       'Dimension équipements réseau — OLT, ONT, switches',    3840),
    ('dim_localisation', 'conformed',       'Dimension géographique — wilayas, daïras, communes',   1542),
    ('dim_technicien',   'slowly_changing', 'Dimension techniciens terrain — personnel habilité',    248),
]
for tname, dim_type, tdesc, trows in dim_specs:
    ...
    dw_tables[tname] = t
    if c: print(f"  ✓ {tname} [DIMENSION]")

# Proxy model instances
...
for ft, mname, mcol, magg, munit, mfmt in measures_spec:
    m, c = Measure.objects.get_or_create(
        ...
        defaults=dict(column=mcol, aggregation_type=magg, alias=mname,
                      description=f'Agrégation {magg.upper()} sur {mcol}',
...
pipelines_spec = [
    dict(name='ETL Incidents Reseau vers DW',
         description='Extrait les incidents depuis la BD Production, normalise (déduplication, enrichissement géographique) et charge dans fact_incidents_reseau.',
         pipeline_type='etl', status='active', source=src_pg,
         schedule_enabled=True, schedule_frequency='daily', schedule_cron='0 2 * * *',
         priority=1, category='Réseau', tags=['incidents', 'réseau', 'critique'],
    ...
    dict(name='ETL Consommation Bande Passante vers DW',
         description='Collecte les métriques IoT de bande passante, agrège par nœud/jour et charge dans fact_consommation_bp.',
         pipeline_type='etl', status='active', source=src_iot,
         schedule_enabled=True, schedule_frequency='hourly', schedule_cron='0 * * * *',
         priority=2, category='Performance', tags=['bande-passante', 'iot', 'métriques'],
    ...
    dict(name='ETL Interventions Techniques vers DW',
         description='Synchronise les interventions terrain, calcule MTTR et taux de résolution, alimente fact_interventions.',
         pipeline_type='etl', status='active', source=src_pg,
    ...
    dict(name='ETL Alertes Securite Kafka vers DW',
         description='Consomme le topic Kafka des alertes sécurité, détecte les anomalies et charge dans la table de faits sécurité.',
         pipeline_type='etl', status='active', source=src_kafka,
         schedule_enabled=True, schedule_frequency='realtime', schedule_cron='',
         priority=1, category='Sécurité', tags=['alertes', 'sécurité', 'kafka', 'critique'],
    ...
    dict(name='ETL Clients CSV vers DW Dimension',
         description='Charge les exports CSV CRM dans dim_client avec gestion SCD Type 2 pour l\'historique des changements d\'offre.',
...
# ─────────────────────────────────────────────────────────────
# 9. JOURNAUX D'EXÉCUTION
# ─────────────────────────────────────────────────────────────
print("\n[9/14] Journaux d'exécution...")

etl_inc  = pipelines['ETL Incidents Reseau vers DW']
etl_cons = pipelines['ETL Consommation Bande Passante vers DW']
etl_alrt = pipelines['ETL Alertes Securite Kafka vers DW']

exec_count = 0

if not ExecutionLog.objects.filter(pipeline=etl_inc).exists():
    history = [
        # (days_ago, status, triggered_by, rows_r, rows_w, duration, error)
        (0, 'completed', 'schedule', 28640, 28597, 142.3, ''),
        (1, 'completed', 'schedule', 28512, 28512, 138.7, ''),
        (2, 'failed',    'schedule', 15000,     0,  45.2,
         'ConnectionError: Timeout 30s sur 192.168.224.128:5432 — vérifiez la disponibilité du serveur PostgreSQL.'),
    ...
    for days, status, trig, rr, rw, dur, err in history:
        ...
        ExecutionLog.objects.create(
            ...
            transformation_logs=[
                ...
                {'transformation': 'Normalisation sévérité',     'status': 'ok', 'timestamp': (started + timedelta(seconds=10)).isoformat(), 'details': {}},
                {'transformation': 'Enrichissement géographique','status': 'ok', 'timestamp': (started + timedelta(seconds=30)).isoformat(), 'details': {}},
            ] if status == 'completed' else [],
        )
        exec_count += 1
    print(f"  ✓ {len(history)} exécutions pour '{etl_inc.name}'")
...
if not ExecutionLog.objects.filter(pipeline=etl_cons).exists():
    for h in range(0, 25, 6):
        ...
        ExecutionLog.objects.create(
            ...
            transformation_logs=[{'transformation': 'Agrégation horaire→journalière', 'status': 'ok',
                                   'timestamp': started.isoformat(), 'details': {}}],
        )
        exec_count += 1
    # Une exécution en cours
    ExecutionLog.objects.create(
        pipeline=etl_cons,
    ...
    exec_count += 1
    print(f"  ✓ Exécutions pour '{etl_cons.name}' (dont 1 en cours)")
...
if not ExecutionLog.objects.filter(pipeline=etl_alrt).exists():
    ...
    exec_count += 2
    print(f"  ✓ Exécutions pour '{etl_alrt.name}' (1 échouée, 1 réessai OK)")

print(f"  Total : {exec_count}+ logs créés")

# ─────────────────────────────────────────────────────────────
# 10. SCHÉMAS DIMENSIONNELS (star_schema app)
# ─────────────────────────────────────────────────────────────
print("\n[10/14] Schémas dimensionnels...")

# NOTE: name doit correspondre à ^[a-zA-Z_][a-zA-Z0-9_]*$
schema_inc, c = DimensionalSchema.objects.get_or_create(
    name='schema_incidents_reseau',
    defaults=dict(
        description='Schéma en étoile — incidents réseau fibre reliés aux dimensions temps, localisation, équipement et client.',
        schema_type='star', status='active', version='2.1',
        grain='transaction', default_join_type='left',
        category='Réseau', business_domain='Opérations Réseau',
        tags=['incidents', 'réseau', 'star'],
        measure_config=[
...
if c: print("  ✓ schema_incidents_reseau")

schema_cons, c = DimensionalSchema.objects.get_or_create(
    name='schema_consommation_bp',
    defaults=dict(
        description='Schéma en étoile — consommation bande passante par nœud réseau agrégée quotidiennement.',
        schema_type='star', status='active', version='1.3',
        grain='daily', default_join_type='left',
        category='Performance', business_domain='Infrastructure Réseau',
...
if c: print("  ✓ schema_consommation_bp")

schema_int, c = DimensionalSchema.objects.get_or_create(
    name='schema_interventions_techniques',
    defaults=dict(
        description='Schéma en flocon — interventions terrain enrichies par les dimensions technicien, incident et localisation.',
...
if c: print("  ✓ schema_interventions_techniques")
...
dashboards_spec = [
    dict(slug='operationnel-reseau',
         name='Tableau de Bord Opérationnel Réseau',
         description='Vue opérationnelle temps réel du réseau fibre SOTIFibre : incidents actifs, disponibilité, bande passante et alertes.',
         dashboard_type='operational', theme='dark',
         refresh_frequency='5min', auto_refresh=True,
         category='Réseau', access_level='team', view_count=1284,
         tags=['opérationnel', 'réseau', 'temps-réel']),
    dict(slug='performance-kpis-reseau',
         name='Performance et KPIs Réseau SOTIFibre',
         description='Indicateurs clés de performance réseau fibre : taux de disponibilité, MTTR, satisfaction client et évolution des débits.',
    ...
    dict(slug='incidents-fibre-optique',
         name='Suivi des Incidents Fibre Optique',
         description='Analyse détaillée des incidents réseau fibre : distribution géographique, causes, évolution temporelle et efficacité des équipes.',
         dashboard_type='analytical', theme='light',
         refresh_frequency='hourly', auto_refresh=True,
         category='Qualité', access_level='team', view_count=456,
         tags=['incidents', 'analyse', 'qualité']),
    dict(slug='consommation-bp-zone',
         name='Consommation Bande Passante par Zone',
         description='Cartographie et analyse de la consommation bande passante par zone géographique, nœud et type d\'offre.',
         dashboard_type='analytical', theme='light',
         refresh_frequency='hourly', auto_refresh=True,
         category='Réseau', access_level='team', view_count=321,
         tags=['bande-passante', 'géographie', 'réseau']),
]
...
db_op  = dashboards['operationnel-reseau']
db_kpi = dashboards['performance-kpis-reseau']
db_inc = dashboards['incidents-fibre-optique']
db_bp  = dashboards['consommation-bp-zone']
...
kpis_spec = [
    # (dashboard, name, kpi_type, schema, measure, cur, target, warn, crit, unit, dec, trend, desc)
    (db_op,  'Disponibilité Réseau',         'percentage', schema_inc,  measures['Taux Resolution Incidents'],
     99.2, 99.0, 99.5, 95.0, '%', 1, 'up',
     'Disponibilité actuelle du réseau fibre SOTIFibre'),
    (db_op,  'Incidents Actifs',             'number',     schema_inc,  measures['Nombre Incidents'],
     12.0, None, 20.0, 30.0, 'Incidents', 0, 'down',
     'Nombre d\'incidents réseau en cours de traitement'),
    (db_op,  'Bande Passante Utilisée',      'percentage', schema_cons, measures['Taux Utilisation Reseau'],
     73.4, None, 80.0, 90.0, '%', 1, 'down',
     'Taux moyen d\'utilisation de la bande passante réseau'),
    (db_op,  'MTTR - Temps Moyen Réparation','number',     schema_int,  measures['Duree Moyenne Intervention'],
     4.2, None, 6.0, 8.0, 'Heures', 1, 'down',
     'Durée moyenne de résolution d\'un incident réseau'),
    (db_kpi, 'Clients Actifs Fibre',         'number',     schema_int,  None,
     42380.0, 45000.0, None, None, 'Abonnés', 0, 'up',
     'Nombre total d\'abonnés fibre actifs chez SOTIFibre'),
    (db_kpi, 'Taux de Satisfaction Client',  'percentage', schema_int,  None,
     87.3, 90.0, 85.0, 75.0, '%', 1, 'up',
     'Score de satisfaction client — enquêtes mensuelles'),
    (db_kpi, 'Revenu Mensuel Fibre',         'currency',   schema_int,  None,
     18_450_000.0, 20_000_000.0, None, None, 'DA', 0, 'up',
     'Chiffre d\'affaires mensuel des abonnements fibre'),
    (db_kpi, 'Débit Moyen Réseau',           'number',     schema_cons, measures['Debit Descendant Moyen'],
     287.4, None, None, None, 'Mbps', 1, 'up',
     'Débit descendant moyen observé sur l\'ensemble du réseau'),
    (db_inc, 'Incidents Résolus Aujourd\'hui','number',     schema_inc,  measures['Nombre Incidents'],
     34.0, None, None, None, 'Incidents', 0, 'up',
     'Nombre d\'incidents clôturés dans la journée'),
    (db_inc, 'Incidents Critiques En Cours', 'number',     schema_inc,  measures['Nombre Incidents'],
     3.0, None, 5.0, 10.0, 'Critiques', 0, 'down',
     'Incidents de sévérité critique actuellement ouverts'),
    (db_inc, 'Taux Résolution 24h',          'percentage', schema_inc,  measures['Taux Resolution Incidents'],
     91.2, 95.0, 85.0, 70.0, '%', 1, 'up',
     'Pourcentage d\'incidents résolus en moins de 24 heures'),
    (db_inc, 'Temps Moyen Détection',        'number',     schema_inc,  None,
     8.4, None, 15.0, 30.0, 'Minutes', 1, 'down',
     'Délai moyen entre apparition et détection d\'un incident'),
    (db_bp,  'Bande Passante Totale Utilisée','number',    schema_cons, measures['Data Transferee Totale'],
     1842.7, None, None, None, 'To/mois', 1, 'up',
     'Volume total de données transférées sur le réseau ce mois'),
    (db_bp,  'Nœuds en Saturation',          'number',     schema_cons, measures['Taux Utilisation Reseau'],
     7.0, None, 10.0, 20.0, 'Nœuds', 0, 'down',
     'Nombre de nœuds réseau avec taux d\'utilisation > 90%'),
]
...
for db, rname, rfmt, rsched, recips, rtags in reports_spec:
    if not Report.objects.filter(name=rname).exists():
        ...
        rcount += 1
print(f"  ✓ {rcount} rapports")

notifs_spec = [
    (admin_user,   'pipeline_complete', 'Pipeline ETL terminé avec succès',
     'Le pipeline "ETL Incidents Reseau vers DW" a traité 28 597 lignes en 142s. Score qualité : 91/100.',
     'medium', etl_inc, None, None, 'delivered', 1),
    (admin_user,   'pipeline_failed', 'Échec Pipeline — Action requise',
     'Le pipeline "ETL Alertes Securite Kafka vers DW" a échoué. Erreur : Leader Not Available. Vérifiez le broker Kafka.',
     'high', etl_alrt, None, None, 'pending', 0),
    (analyst_user, 'kpi_alert', 'KPI Incidents Critiques — Seuil approché',
     'Le KPI "Incidents Critiques En Cours" atteint 3 incidents (seuil avertissement : 5). Surveillance recommandée.',
     'medium', None, None, None, 'pending', 0),
    (director_user,'report_ready', 'Rapport mensuel disponible',
     'Le rapport "Rapport Mensuel Performances Réseau" est prêt. Téléchargez le PDF depuis la plateforme.',
     'low', None, None, None, 'delivered', 1),
    (admin_user,   'data_refresh', 'Rafraîchissement données terminé',
     'La source "Base de Données Production SOTIFibre" a été synchronisée — 185 420 lignes indexées.',
     'low', None, None, None, 'delivered', 1),
    (dev_user,     'anomaly_detected', 'Anomalie — Débit Réseau',
     'OLT-ALGER-03 : débit montant 847 Mbps vs moyenne 287 Mbps (+195%). Vérification recommandée.',
     'high', None, None, None, 'pending', 0),
    (analyst_user, 'kpi_target_reached', 'Objectif KPI atteint — Disponibilité',
     'Le KPI "Disponibilité Réseau" atteint 99.2%, dépassant l\'objectif de 99.0%. Bravo à l\'équipe terrain !',
     'low', None, None, None, 'read', 1),
    (admin_user,   'system_alert', 'Alerte Disque Data Warehouse',
     'Espace disque sotifibre_dw : 2.45 Go / 3 Go (81.7%). Prévoir une extension de capacité.',
     'high', None, None, None, 'pending', 0),
]
...
print(f"  Sources données  : {DataSource.objects.count()}")
print(f"  Tables données   : {DataTable.objects.count()}")
print(f"  Schémas DW       : {DataWarehouseSchema.objects.count()}")
...
print(f"  Exécutions ETL   : {ExecutionLog.objects.count()}")
print(f"  Schémas Dim.     : {DimensionalSchema.objects.count()}")
